:PROPERTIES:
:ID:       e202e4bc-a7ac-4e0c-8fbd-ca9ce1707fae
:END:
...
* Clone both the files at once
#+begin_src emacs-lisp
(fset 'emacs-init-and-org-file-close-at-once
   (kmacro-lambda-form [?\C-q ?\C-q ?\C-x ?1] 0 "%d"))
global-set-key (kbd "C-c C-c") 'emacs-init-and-org-file-close-at-once)
#+end_src
